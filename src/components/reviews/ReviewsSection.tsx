  ...
  return (
    <form onSubmit={submit} className="space-y-5">
      ...
      <div>
        ...
        <textarea id="review-text" value={reviewText} onChange={(event) => setReviewText(event.target.value)} maxLength={2000} rows={5} placeholder={t('reviewPlaceholder')} className="w-full resize-y rounded-md border border-input bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" />
      ...
      <div>
        ...
        <input id="review-category" value={category} onChange={(event) => setCategory(event.target.value)} maxLength={80} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" />
      </div>
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="min-w-0">
            <div className="mb-5 flex flex-col gap-3 md:flex-row"><form onSubmit={(event) => { event.preventDefault(); setPage(1); setSearch(searchInput.trim()); }} className="relative flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={t('searchReviews')} className="h-10 w-full rounded-md border border-input bg-background pl-9 pr-3 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring" /></form><select value={ratingFilter ?? ''} onChange={(event) => { setPage(1); setRatingFilter(event.target.value ? Number(event.target.value) : undefined); }} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="">{t('all')}</option>{[5, 4, 3, 2, 1].map((rating) => <option key={rating} value={rating}>{rating} {t('stars')}</option>)}</select><select value={sort} onChange={(event) => { setPage(1); setSort(event.target.value as SortOrder); }} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="recent">{t('mostRecent')}</option><option value="highest">{t('highestRated')}</option><option value="lowest">{t('lowestRated')}</option><option value="helpful">{t('mostHelpful')}</option></select></div>
