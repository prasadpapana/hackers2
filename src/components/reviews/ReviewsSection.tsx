      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          ...
          <div className="min-w-0">
            <div className="mb-5 grid gap-3 md:grid-cols-[minmax(0,1fr)_8rem_9rem]"><form onSubmit={(event) => { event.preventDefault(); setPage(1); setSearch(searchInput.trim()); }} className="relative min-w-0"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder={t('searchReviews')} aria-label={t('searchReviews')} className="h-10 pl-9" /></form><Select value={ratingFilter?.toString() ?? ''} onChange={(event) => { setPage(1); setRatingFilter(event.target.value ? Number(event.target.value) : undefined); }} options={[{ value: '', label: t('all') }, ...[5, 4, 3, 2, 1].map((rating) => ({ value: rating.toString(), label: `${rating} ${t('stars')}` }))]} className="w-full" /><Select value={sort} onChange={(event) => { setPage(1); setSort(event.target.value as SortOrder); }} options={[{ value: 'recent', label: t('mostRecent') }, { value: 'highest', label: t('highestRated') }, { value: 'lowest', label: t('lowestRated') }, { value: 'helpful', label: t('mostHelpful') }]} className="w-full" /></div>
