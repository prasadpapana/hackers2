  ...
  return (
    <DashboardLayout>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        {loading ? <LoadingState message="Loading case details..." /> : error || !caseData ? (
          <ErrorState title="Case unavailable" message="We could not load this case from the server." onRetry={() => window.location.reload()} />
        ) : (
          <>
            ...
            <div className="grid lg:grid-cols-3 gap-6">
              ...
              <Card>
                ...
                <CardContent className="flex items-center gap-2"><Calendar className="w-4 h-4 text-primary" /><p className="text-foreground">{caseData.deadline ?? 'No deadline set'}</p></CardContent>
              </Card>
            </div>
            ...
            <div className="grid lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-3">
                  {caseData.documents.length === 0 ? <p className="text-muted-foreground">No documents attached.</p> : caseData.documents.map((document) => (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  {caseData.timeline.length === 0 ? <p className="text-muted-foreground">No timeline events yet.</p> : caseData.timeline.map((event) => (
